...
const CustomSelect = ({options, value, onChange, title, width, getLabel}: CustomSelectProps) => {
        ...
        <div ref={ref} className={styles.container}>
            ...
            {isOpen && (
                <ul className={styles.selector_list}>
                    {options.map(option => (
                        <li className={`${styles.selector_elem} ${getLabel(value) === option.label ? styles.selector_elem_active : false}`} key={option.value} onClick={() => handleCustomSelect(option)}>
                            {option.label}
                        </li>
                    ))}
                </ul>
